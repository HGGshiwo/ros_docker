...
*   **桌面终端与多窗口自启动**：
    您可以通过编辑 [my_terminal_autostart/autostart.sh](file:///home/hggshiwo/catkin_ws/ros_docker/my_terminal_autostart/autostart.sh) 脚本来管理在 VNC 桌面启动时需要开辟的多个 `terminator` 窗口以及需要自动执行的 ROS 节点/指令。
    *系统已预配置好了一个启动 `roscore` 和 `bridge_mavlink_node` 双终端的示例。请确保 [launch.desktop](file:///home/hggshiwo/catkin_ws/ros_docker/my_desktop_autostart/launch.desktop) 的执行目标指向此脚本。*
...
需要在start_docker.sh之前执行：

```
docker run -p 5760:5760  -it orthuk/ardupilot-sitl-debian:0.2.0 ./Tools/autotest/sim_vehicle.py -v Copter --no-mavproxy --no-rebuild
```